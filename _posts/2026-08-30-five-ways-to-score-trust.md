...
Here is what that formula actually produces at one of the walk-forward cutoffs. Mean trust sits at 49.06% and the median is exactly 50%, which makes sense since the formula is centered on chance. The interesting part is the tail: 3,330 authors sit at the exact maximum score, with a median call count of only 6, one above the minimum of 5 calls required to be scored at all. A handful of calls that all happen to be correct produces a perfect looking score. That is the same small sample problem behind the min_calls threshold, and it is a good argument for testing whether 5 is actually high enough.

![Trust score distribution at one walk-forward cutoff, min_calls 5](/assets/images/trustlens-trust-scores.png)
